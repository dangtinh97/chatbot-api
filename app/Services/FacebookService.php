...
class FacebookService
{
    ...
    {
        if ($postback['title'] === "Get Started" || $postback == "Bắt đầu") {
    ...
    public function connect($user): ApiResponse
    {
        ...
        if ($connect->status === Connect::STATUS_BUSY) {
            FChatHelper::sendMessageText($user->fb_uid,
                $this->buttonMenu("Bạn đang trong cuộc trò chuyện với ai đó.", Connect::STATUS_BUSY));
        ...
        if ($connect->status === Connect::STATUS_FIND) {
            FChatHelper::sendMessageText($user->fb_uid,
                $this->buttonMenu("Bạn đang trong hàng đợi.", Connect::STATUS_FIND));
        ...
        if (is_null($userFind)) {
            FChatHelper::sendMessageText($user->fb_uid,
                $this->buttonMenu("Chúng tớ đang tìm người phù hợp với bạn!", Connect::STATUS_FIND));
        ...
        FChatHelper::sendMessageText($user->fb_uid,
            $this->buttonMenu("Bạn đã được kết nối với người lạ!", Connect::STATUS_BUSY));
    ...
    public function disconnect($user): ApiResponse
    {
        ...
        if ($connect->status === Connect::STATUS_FREE) {
            FChatHelper::sendMessageText($user->fb_uid,
                $this->buttonMenu("Bạn chưa kết nối với ai!", Connect::STATUS_FREE));
        ...
        if ($status === Connect::STATUS_FIND) {
            FChatHelper::sendMessageText($user->fb_uid,
                $this->buttonMenu("Bạn đã rời khỏi hàng đợi!", Connect::STATUS_FREE));
        ...
        if (!is_null($userConnected)) {
            FChatHelper::sendMessageText($userConnected->user->fb_uid,
                $this->buttonMenu("Người lạ đã ngắt kết nối với bạn.", Connect::STATUS_FREE));
        ...
        FChatHelper::sendMessageText($user->fb_uid,
            $this->buttonMenu("Bạn đã ngắt kết nối với người lạ!", Connect::STATUS_FREE));
                        ...
                        [
                            ...
                            'buttons' => [
                                ...
                                FChatHelper::buttonUrl("https://tool.nguoila.online/game/dino?fb_uid=".$this->fbUid
                                    ."?utm_source=haui_chatbot", "Khủng long chạy bộ"),
                                FChatHelper::buttonUrl("https://tool.nguoila.online/caro-online?fb_uid=".$this->fbUid
                                    ."?utm_source=haui_chatbot", "Chơi Caro online"),
                            ]
                        ]
                    ]
                ]
